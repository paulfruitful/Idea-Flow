...
<div class="relative flex flex-row m-6 p-6 text-center"><h1 class="font-bold text-3xl">Share Your Problem</h1></div>
  <form action="/problem/{{$problem->id}}/edit" class="block  p-12 m-6 border-solid border-2  shadow-xl  rounded-md bg-white" method="POST" enctype="multipart/form-data">
     @csrf
@if($errors->all())
...
<div class="relative flex flex-row m-6">
  ...
  <input type="text" placeholder="Name e.g MediStar" class=" placeholder:italic placeholder:text-slate-400 block border-solid border-t-0 border-b-2 border-slate-600  focus:outline-none w-full focus:border-sky-500" style=" border-bottom-width:2.4px;" name="title" value={{$problem->title}}>
</div>
...
<div class="relative flex flex-col m-6 mt-6">
  <textarea placeholder="Idea to solve..." id="edit" class="  placeholder:italic placeholder:text-slate-400 block border-solid border-2 p-6 border-slate-400  focus:outline-none focus:border-sky-500" name="description">{{$problem->description}}</textarea>
